...
class RemoteMessages {
  ///====zola: modify add arg
  constructor (isTargetBased = false, zola_testRemoteDebugAppDic = null) {
    this.isTargetBased = isTargetBased;
    this.zola_testRemoteDebugAppDic = zola_testRemoteDebugAppDic;
  }

  ///====zola: modify add method
  zola_updateRemoteDebugAppDic(appDic = {}) {
    this.zola_testRemoteDebugAppDic = appDic;
    return;
  }
  ...
  getRemoteCommand (command, opts) {
    ...
    switch (command) {
      ...
      case 'enablePage':
        method = 'Page.enable';
        // debugger; ///====zola: modify jsc 直接替换 Console.enable
        if (this.zola_testRemoteDebugAppDic && this.zola_testRemoteDebugAppDic[appIdKey]) {
          for (const dict of this.zola_testRemoteDebugAppDic[appIdKey].pageArray) {
            if (dict.id === pageIdKey) {
              console.log(`========================zola:enablePage change to console.enable forDic=appid:${appIdKey}.${pageIdKey}, ${JSON.stringify(dict)}`)
      ...
      case 'startNetwork':
        ...
        if (this.zola_testRemoteDebugAppDic && this.zola_testRemoteDebugAppDic[appIdKey]) {
          for (const dict of this.zola_testRemoteDebugAppDic[appIdKey].pageArray) {
            if (dict.id === pageIdKey) {
              console.log(`========================zola:Network.enable change to console.enable forDic=appid:${appIdKey}.${pageIdKey}, ${JSON.stringify(dict)}`)
              method = 'Console.enable';
              break;
            }
          }
