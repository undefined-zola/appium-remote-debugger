...
///===zola: add method 获取JSC Context
function zola_isJSContextTitle(pageDict) {
  if (!pageDict || !(pageDict.WIRTitleKey || pageDict.title)) {
    return false;
  }
  const pageTitle = pageDict.WIRTitleKey || pageDict.title;
  console.log(`========================zola:  zola_isJSContextTitle: check is JSCore Context ${pageTitle}`);
  return (/^(AppletCtx|GameMainContext|GameOpenDataContext)/.test(pageTitle)) && (pageTitle.indexOf("bili") != -1) ;
}
...
function pageArrayFromDict (pageDict) {
  ...
  for (const dict of _.values(pageDict)) {
    // debugger;
    const isJSContext = zola_isJSContextTitle(dict);
    if (_.isUndefined(dict.WIRTypeKey) || dict.WIRTypeKey === 'WIRTypeWeb' || (dict.WIRTypeKey === 'WIRTypeJavaScript' && isJSContext)) {
      let url = dict.WIRURLKey;
      if (isJSContext) {
        url = "JSCContext://" + dict.WIRTitleKey;
        console.log(`==================zola: modify JSCore context page url:${url}`);
      }
      // debugger;
    // count only WIRTypeWeb pages and ignore all others (WIRTypeJavaScript etc)
    // if (_.isUndefined(dict.WIRTypeKey) || dict.WIRTypeKey === 'WIRTypeWeb') { ///===zola: modify
      newPageArray.push({
...
async function getScriptForAtom (atom, args, frames, asyncCallBack = null, zola_pageDict = null) {
  let isJSCContextTitle = zola_isJSContextTitle(zola_pageDict);
  console.log(`=======================zola:help.js:getScriptForAtom:${JSON.stringify(args)}, async:${asyncCallBack}, page:${JSON.stringify(zola_pageDict)}, isjsc:${isJSCContextTitle}`);
  // debugger;
  if (isJSCContextTitle) {
    let script;
    if (asyncCallBack) {
      args = args.map(JSON.stringify);
      script += `(${args.join(',')}, ${asyncCallBack}, true )`;
    } else {
      script = `(function(){return ${args[0]}; })()`;
    }
    console.log(`========================zola: getScriptForAtom:modify jsc after pageurl:${zola_pageDict.url}, script:${script}`);
    return script;
  }  
  console.log(`==========================zola:help.js:getScriptForAtom do with web: ${JSON.stringify(args)}, async:${asyncCallBack}, page:${JSON.stringify(zola_pageDict)}`);
