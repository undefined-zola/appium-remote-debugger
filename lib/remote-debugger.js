...
class RemoteDebugger extends events.EventEmitter {
  ...
  updateAppsWithDict (dict) {
    ...
    console.log(`============================zola remote-debugger.js try updateAppsWithDict update rpcClient debugAppDic`);
    this.rpcClient.zola_updateAppsWithDict(this.appDict);
  }
  ...
  async executeAtom (atom, args, frames) {
    ...
    // modify by ===zola: add pageDict
    // debugger;
    let pageDict;
    for (const dict of this.appDict[this.appIdKey].pageArray) {
      if (dict.id === this.pageIdKey) {
        pageDict = dict;
        break;
      }
    }
    console.log(`============================zola: remote-debugger: excuteAtom:${this.appIdKey}.${pageDict.id}`);
    const script = await getScriptForAtom(atom, args, frames, undefined, pageDict);
    // debugger;
    // const script = await getScriptForAtom(atom, args, frames);   ///=====zola: modify
    const value = await this.execute(script, true);
    console.log(`============================zola: remote-debugger: excuteAtom  \n script:${script} \n result:${value}`);
  ...
  async checkPageIsReady () {
    ...
    try {
      // debugger; ///zola: modify jsc ducoument 检查默认通过
      let pageArray = this.appDict[this.appIdKey].pageArray;
      let isJSCContext = false;
      for (const page of pageArray) {
        if (this.pageIdKey == page.id) {
          if (page.url.indexOf('JSCContext')>=0) {
            isJSCContext = true;
          }
          break;
        }
